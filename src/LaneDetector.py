...
class LaneDetector():
    ...
    def __init__(self, width: int, height: int, canny_th1=60, canny_th2=75, 
                 roi_start_row=300, roi_end_row=600, roi_start_column = 0, roi_end_column = 1280, margin=300, lane_interval=660):
        self.WIDTH, self.HEIGHT = width, height
        self.ROI_START_ROW = roi_start_row          # roi의 y 시작
        self.ROI_END_ROW = roi_end_row              # roi의 y 끝
        self.ROI_START_COLUMN = roi_start_column          # roi의 x시작
        self.ROI_END_COLUMN = roi_end_column              # roi의 x 끝
    ...
    def pre_process(self, img):
        roi_img = img[self.ROI_START_ROW:self.ROI_END_ROW, self.ROI_START_COLUMN:self.ROI_END_COLUMN]
        binary_img = self.binarization(roi_img)
        edge_img = cv2.Canny(np.uint8(binary_img), self.CANNY_TH1, self.CANNY_TH2)
    ...
    def do(self, img, display_img = False):
        ...
        if m_left != 0.0 and m_right != 0.0:
            if x_right - x_left > self.LANE_INTERVAL + 100: # arbitrarily chosen value (100)
                 m_left = 0.0
